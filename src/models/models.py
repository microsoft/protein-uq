...
class LinearVariational(nn.Module):
    def __init__(
        ...
        self.w_mu = nn.Parameter(
            torch.FloatTensor(in_features, out_features).normal_(mean=0, std=0.01)
        )
        # proxy for variance
        # log(1 + exp(ρ))*eps
        self.w_p = nn.Parameter(
            torch.FloatTensor(in_features, out_features).normal_(mean=-5, std=0.01)  # rho=-5 gives sigma=0.003 through softmax
        )
        if self.include_bias:
            self.b_mu = nn.Parameter(torch.zeros(out_features))
            self.b_p = nn.Parameter(torch.zeros(out_features) - 5)  # rho=-5 gives sigma=0.003 through softmax
...
class FluorescenceModel(nn.Module):
    def __init__(
        ...
        self.esm_conv = nn.Conv1d(
            1, 1, kernel_size, padding=2
    ...
    def forward(self, x, mask, evidential=False):
        ...
        if self.input_type == "ohe":
            ...
            x = F.relu(self.esm_conv(
                x.unsqueeze(1)
            ))  # unsqueeze to make shape be [batch_size, 1, 1280]
        elif self.input_type == "esm_full":
            raise ValueError(
                "ESM full not implemented yet"
            )  # TODO: implement this option using attention layer defined above
        # embed
        x = self.embedding(x)
        x = self.dropout(x)
        # decoder
        ...
        if evidential:
            ...
            lambdas = F.softplus(loglambdas) + min_val  # also called nu or v
            alphas = (
                F.softplus(logalphas) + min_val + 1
            ...
            means, vars = torch.split(
                output, output.shape[1] // 2, dim=1
            )
            vars = F.softplus(vars)  # constrain variance to be positive
            output = torch.stack((means, vars), dim=2).view(
                output.size()
            )
